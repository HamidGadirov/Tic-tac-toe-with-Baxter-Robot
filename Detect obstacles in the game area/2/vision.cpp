...
int Vision::detectTkn(int cellnum, Board brd) {
    cv::Mat pic = rgb.clone();
    cv::Mat T = squareTransformation(brd.getCornerImgPoints(), pic);

    cv::Mat input_transformed = rgb.clone();
    cv::warpPerspective(rgb, input_transformed, T, input_transformed.size());

    cv::Rect brect = cv::boundingRect(brd.getCornerImgPoints());
    int size = std::max(brect.width, brect.height);

    // final squared board
    cv::Mat region = input_transformed(cv::Rect(brect.x, brect.y, size, size));
...
int Vision::detectNextMove() {
    ...
    cv::Mat input_img = board.region.clone(); // background image
    // Vision::step() in while loop ?
    cv::Mat diff;

    while(true) { // update final squared board here, each second
        // wait 1 s ?
        //#include <unistd.h>
        //usleep(1000000);
        ...
        cv::Mat new_img = region.clone(); // new final squared board image
